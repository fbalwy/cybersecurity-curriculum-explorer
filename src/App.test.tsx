...
describe('curriculum focus interactions', () => {
  ...
  it('opens every course in direct-unlocks mode and offers an explicit back action', () => {
    const view = render(<App />)

    fireEvent.click(view.container.querySelector('[data-course-code="CS114"]')!)

    expect(screen.getByRole('region', { name: 'Focused courses it unlocks directly for CS114' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Courses it unlocks directly' })).toHaveAttribute('aria-pressed', 'true')
    ...
    expect(window.location.search).toContain('mode=direct-unlocks')
